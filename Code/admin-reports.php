...
$querysup = "UPDATE admins SET timeVisite=CURRENT_TIME() WHERE admin='admin'";
...
<html>
    <head>
    <title>Admin reports</title><link rel="icon" href="Img/Edu.svg">
...
<div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Table de reports</strong>
                            </div>
                            <div class="row">
                            ...
                            <div class="card-body">
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody id="myList">
                                    <?php
      $queryrep = "SELECT * FROM reports ORDER BY Time DESC LIMIT $page_first_result,$results_per_page";
      $resultsrep = mysqli_query($db, $queryrep);
      while($rowrep = $resultsrep->fetch_array()){
        
          echo '<tr>
          <td>'.$rowrep['reportID'].'</td>
          <td>'.$rowrep['reporterID'].'</td>
          <td>'.$rowrep['typeR'].'</td>
          <td>'.$rowrep['typeID'].'</td>
          <td>'.$rowrep['Fil'].'</td>
          
                 <td><form action="" method="post">
          <button class="btn btn-primary bg-secondary" id="Ign" name="Ign" value="'.$rowrep['reportID'].'" type="submit">Ignorer</button>
                 </form>
                 </td>
          </tr>';
...
<?php if ($p!=1): ?>
<a href="admin-reponses?reports=p=<?php echo $p-1;?>">&laquo;</a>
<?php endif ?>
<?php for($page = 1; $page<= $number_of_page; $page++) {  
    
    if($p==$page)
    echo '<a class="active" href="admin-reports?p='.$page.'">'.$page.'</a>';
else echo '<a href="admin-reports?p='.$page.'">'.$page.'</a>'; }
 ?>
 <?php if ($p!=$number_of_page): ?>
  <a href="admin-reponses?reports=p=<?php echo $p+1;?>">&raquo;</a>
  <?php endif ?>
</div>
</div>
                    </div>
...
<?php if (isset($_POST['Ign']))
{
  $db = mysqli_connect('127.0.0.1', 'root', '', 'pfe');
  $idr=$_POST["Ign"];
  $querysup = "DELETE FROM reports WHERE reportID='$idr'";
mysqli_query($db, $querysup);
