                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody id="myList">
                                    <?php
      $queryque = "SELECT * FROM fileupload ORDER BY Time DESC LIMIT $page_first_result,$results_per_page";
      $resultsque = mysqli_query($db, $queryque);
      while($rowque = $resultsque->fetch_array()){
        
          echo '<tr>
         
          <td>'.$rowque['file_name'].'</td>
          <td>'.$rowque['GroupID'].'</td>
          <td>'.$rowque['Fillier'].'</td>
          <td>'.$rowque['StudentID'].'</td>
   
          <td><form action="" method="post">
          <button class="btn btn-primary bg-secondary" id="Sup" name="Sup" value="'.$rowque['file_name'].'" type="submit">Supprimer</button>
                 </form>
                 </td>
...
<?php if (isset($_POST['Sup']))
{
  $db = mysqli_connect('127.0.0.1', 'root', '', 'pfe');
  $idq=$_POST["Sup"];
  $querysup = "DELETE FROM fileupload WHERE file_name='$idq'";
