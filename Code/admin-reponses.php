<html>
    <head>
    <title>Admin Reponses</title><link rel="icon" href="Img/Edu.svg">
...
$queryC = "SELECT * FROM reponse join groups where reponse.groupID=groups.groupID AND groups.Fillier='$F'";  
...
<div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Table de Reponses:</strong>
                            </div>
                            <div class="row">
                                ...
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>questionID</th>
                                            <th>groupID</th>
                                            <th>group-Filliere</th>
                                            <th>EtudiantID</th>
                                            <th>supprimé</th>
                                            <th>supprimer reponse</th>
                                        </tr>
                                    </thead>
                                    <tbody id="myList">
                                    <?php
      $queryrep = "SELECT * FROM reponse join groups where reponse.groupID=groups.groupID AND groups.Fillier='$filliere'  ORDER BY Time DESC LIMIT $page_first_result,$results_per_page";
      $resultsrep = mysqli_query($db, $queryrep);
      while($rowrep = $resultsrep->fetch_array()){
        
          echo '<tr>
          <td>'.$rowrep['reponseID'].'</td>
          <td>'.$rowrep['reponse'].'</td>
          <td>'.$rowrep['questionID'].'</td>
          <td>'.$rowrep['groupID'].'</td>
          <td>'.$rowrep['Fillier'].'</td>
          <td>'.$rowrep['UserId'].'</td>
          <td>'.$rowrep['Deleted'].'</td>
          <td><form action="" method="post">
          <button class="btn btn-primary bg-secondary" id="Sup" name="Sup" value="'.$rowrep['reponseID'].'" type="submit">Supprimer</button>
                 </form>
                 </td>
...
<?php if ($p!=1): ?>
<a href="admin-reponses?Filliere=<?php echo $F;?>&p=<?php echo $p-1;?>">&laquo;</a>
<?php endif ?>
<?php for($page = 1; $page<= $number_of_page; $page++) {  
    
    if($p==$page)
    echo '<a class="active" href="admin-reponses?Filliere='.$F.'&p='.$page.'">'.$page.'</a>';
else echo '<a href="admin-reponses?Filliere='.$F.'&p='.$page.'">'.$page.'</a>'; }
 ?>
 <?php if ($p!=$number_of_page): ?>
  <a href="admin-reponses?Filliere=<?php echo $F;?>&p=<?php echo $p+1;?>">&raquo;</a>
  <?php endif ?>
</div>
</div>
                    </div><?php }?>
...
<?php if (isset($_POST['Sup']))
{
  $db = mysqli_connect('127.0.0.1', 'root', '', 'pfe');
  $idr=$_POST["Sup"];
  $querysup = "UPDATE reponse SET Deleted=1 WHERE reponseID='$idr'";
